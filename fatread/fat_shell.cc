...
void do_lsdir(const std::vector<std::string> &args) {
    ...
    std::cout << args[0] << ": found " << normal_entries << " normal directory entries"
              << " (ignoring " << (entries.size() - normal_entries) << " long directory entries, deleted entries, etc.)\n";
...
void do_pread(const std::vector<std::string> &args) {
    ...
    bool all_text = true;
    for (char c : buffer) {
        if (!std::isprint(c)) all_text = false;
    }
    ...
    if (all_text) {
        ...
        std::ios_base::fmtflags saved_fmt_flags(std::cout.flags());
        std::cout << "result buffer contains non-ASCII characters, displaying as hexadecimal\n";
