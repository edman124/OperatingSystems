...
void parse_and_run_command(const std::string &command) {
    ...
    for(size_t i=0; i < results.size(); i++)
    {
        if (find(redirInd.begin(), redirInd.end(), i) != redirInd.end())
        {
            ...
            if (results[i].compare("<") == 0)
            {
                // in = open(results[i+1].c_str(), O_RDONLY, S_IRWXU | S_IRWXG | S_IRWXO);
                // if (in<0)
                // {
                //     // perror("error");
                //     close(in);
                ...
                // }
                redirLeftCount += 1;
                commands[commandInd].infd = results[i+1];
                // dup2(in, 0);
                // close(in);
                i++;
            }
            ...
            {
                // out = open(results[i+1].c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);
                ...
                // }
                redirRightCount += 1;
                commands[commandInd].outfd = results[i+1];
                // dup2(out, 1);
                i++;
            }
        }
    ...
    for (size_t i = 0; i < commands.size(); i++)
    {
        ...
        else if (pid == 0)
        {
            ...
            if (commands[i].infd.size() != 0)
            {
                int in = open(commands[i].infd.c_str(), O_RDONLY, S_IRWXU | S_IRWXG | S_IRWXO);
                if (in<0)
                {
                    // perror("error");
                    close(in);
            ...
            if (commands[i].outfd.size() != 0)
            {
                int out = open(results[i+1].c_str(), O_WRONLY | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG | S_IRWXO);
                // cout << "file des: " << out << endl;
                if (out<0)
                {
                    // perror("error");
