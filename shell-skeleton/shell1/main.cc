...
void parse_and_run_command(const std::string &command) {
    ...
    else if (results.size() <= 2*redirInd.size() + pipeInd.size())
    {
        cerr << "invalid command" << endl;
        return;
    }
    ...
    for(size_t i=0; i < results.size(); i++)
    {
        if (find(redirInd.begin(), redirInd.end(), i) != redirInd.end())
        {
            ...
            // if (i+1<results.size())
            // {
            //     cout << "boolean " << (results[i+1].compare("<") == 0) << endl;
            // }
        ...
        else if (find(pipeInd.begin(), pipeInd.end(), i) != pipeInd.end())
        {
            ...
            // args[count] = NULL;
            commandInd++;
            commands.push_back(Command(results.size() + 1 - 2*redirInd.size() - 2*pipeInd.size()));
